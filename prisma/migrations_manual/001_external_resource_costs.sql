...
WITH priced_logs AS (
  SELECT
    ...
    -- For resource-unit-priced calls (e.g. web_search, parallel_search), each log row can bundle
    -- several billed search units; report that unit count instead of the row count.
    CASE
      WHEN l.resource_units_count > 0 THEN l.resource_units_count
      ELSE 1
    END AS billed_calls
...
SELECT
  ...
  step_id,
  task,
  sum(billed_calls)                      AS calls_count,
...
-- Column set changed (company_id inserted, companies_count dropped), which
-- CREATE OR REPLACE VIEW does not allow (it only permits appending columns
-- at the end) — the view must be dropped and recreated.
DROP VIEW IF EXISTS public.v_report_cost_by_step_task;

CREATE VIEW public.v_report_cost_by_step_task AS
SELECT
  l.report_id,
  l.company_id,
  l.step_id,
  ...
  l.provider,
  l.model,
  sum(CASE WHEN l.resource_units_count > 0 THEN l.resource_units_count ELSE 1 END) AS total_calls,
...
GROUP BY l.report_id, l.company_id, l.step_id, l.task, l.provider, l.model,
...
CREATE OR REPLACE VIEW public.v_report_cost_by_company AS
SELECT
  v.report_id,
  v.company_id,
  c.name                                AS company_name,
  count(DISTINCT v.step_id)             AS steps_count,
  count(DISTINCT v.task)                AS tasks_count,
  sum(v.calls_count)                    AS total_calls,
  sum(v.calls_without_pricing)          AS calls_without_pricing,
  sum(v.prompt_tokens)                  AS input_tokens,
...
GROUP BY v.report_id, v.company_id, c.name;

-- ----

CREATE OR REPLACE VIEW public.v_report_cost_by_company_step AS
SELECT
  v.report_id,
  v.company_id,
  v.step_id,
...
LEFT JOIN report_generation_steps rgs
  ON rgs.id = v.step_id
LEFT JOIN report_step_statuses rss
  ON rss.step_id = v.step_id AND rss.report_id = v.report_id AND rss.company_id = v.company_id
GROUP BY v.report_id, v.company_id, v.step_id, rs.step_order, rgs.name, rss.status;
