...
export default function AnalyzeServiceExample() {
	...
	const [selectedAnalyzeId, setSelectedAnalyzeId] = useState<string | null>(
		null,
	);
	...
	// Query hooks
	const { data: analysesData, isLoading: isLoadingAnalyzes } =
		useGetAnalyzesByUser(email);
	const analyses = analysesData?.data || [];
	const { data: selectedAnalyze, isLoading: isLoadingSelected } =
		useGetAnalyze(selectedAnalyzeId);
				...
				{latestProgress && (
					...
					>
						...
						<Text style={{ color: "#d9d9d9" }}>
							Company: {latestProgress.companyName} | Status:{" "}
							{latestProgress.status} | Step: {latestProgress.currentStep} |
							Execution: {latestProgress.executionStatus} (
							{latestProgress.executionStep})
						</Text>
					</Card>
				)}
										...
										description={
											<Space orientation="vertical" size="small">
												<Text style={{ color: "#8c8c8c", fontSize: "12px" }}>
													Business: {analyze.businessLine} | Country:{" "}
													{analyze.country}
												</Text>
												<Text style={{ color: "#8c8c8c", fontSize: "12px" }}>
													Status: {analyze.status} | Step: {analyze.currentStep}{" "}
													| Execution: {analyze.executionStatus} (
													{analyze.executionStep})
												</Text>
												<Text style={{ color: "#8c8c8c", fontSize: "12px" }}>
													Created:{" "}
													{new Date(analyze.createdAt).toLocaleDateString()}
												</Text>
											</Space>
										}
									/>
					...
					>
						...
						{isLoadingSelected ? (
							...
							<div style={{ color: "#d9d9d9" }}>
								<p>
									<strong>ID:</strong> {selectedAnalyze._id}
								</p>
								<p>
									<strong>Company:</strong> {selectedAnalyze.companyName}
								</p>
								<p>
									<strong>Business Line:</strong> {selectedAnalyze.businessLine}
								</p>
								<p>
									<strong>Country:</strong> {selectedAnalyze.country}
								</p>
								<p>
									<strong>Use Case:</strong> {selectedAnalyze.useCase}
								</p>
								<p>
									<strong>Timeline:</strong> {selectedAnalyze.timeline}
								</p>
								<p>
									<strong>Status:</strong> {selectedAnalyze.status}
								</p>
								<p>
									<strong>Current Step:</strong> {selectedAnalyze.currentStep}
								</p>
								<p>
									<strong>Execution ID:</strong>{" "}
									{selectedAnalyze.executionId || "N/A"}
								</p>
								<p>
									<strong>Execution Status:</strong>{" "}
									{selectedAnalyze.executionStatus || "N/A"}
								</p>
								<p>
									<strong>Execution Step:</strong>{" "}
									{selectedAnalyze.executionStep || "N/A"}
								</p>
								<p>
									<strong>Created:</strong>{" "}
									{new Date(selectedAnalyze.createdAt).toLocaleString()}
								</p>
								<p>
									<strong>Updated:</strong>{" "}
									{new Date(selectedAnalyze.updatedAt).toLocaleString()}
								</p>
							</div>
						) : (
