...
export default function VitelisSalesCompanyPage() {
  ...
  if (!Number.isFinite(reportId) || !Number.isFinite(companyId)) {
    return (
      <div style={{ minHeight: "100vh", background: "#141414", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Result status="404" title="Company not found" extra={<Button type="primary" onClick={() => router.push("/vitelis-sales")}>Back to list</Button>} />
      </div>
    );
  }

  return <DeepDiveCompany reportId={reportId} companyId={companyId} basePath="/vitelis-sales" />;
}
