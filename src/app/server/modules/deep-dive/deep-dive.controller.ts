...
base AS (
  SELECT
    ...
    c.name AS account,
    COALESCE(co.name, c.name) AS entity,
    ...
    oc.horizon,
    oc.horizon_name,
    oc.horizon_confidence,
    oc.horizon_reasoning,
    oc.horizon_meta->>'indicative_deal_size_range' AS indicative_deal_size_range,
    oc.delivery_type,
    oc.track_1_entity,
    ...
    oc.solution_center,
    CASE
      WHEN jsonb_typeof(oc.meta->'conditional_attach_candidates') = 'array'
        THEN oc.meta->'conditional_attach_candidates'
      ELSE '[]'::jsonb
  ...
  LEFT JOIN companies co
    ON co.id = oc.primary_scope_company_id
),
item_rows AS (
  SELECT
  ...
  FROM base b
  JOIN opportunity_candidate_items oci
  ...
  SELECT
    ...
    string_agg(
      concat(
        ...
        CASE WHEN ir.source_score IS NOT NULL OR ir.source_confidence_score IS NOT NULL THEN ' | scored=yes' ELSE '' END,
        CASE WHEN ir.source_score IS NOT NULL THEN ' | source_score=' || to_char(ir.source_score, 'FM999999990.0000') ELSE '' END,
        CASE WHEN ir.source_confidence_score IS NOT NULL THEN ' | source_confidence=' || to_char(ir.source_confidence_score, 'FM999999990.0000') ELSE '' END
      ),
      E'\\n' ORDER BY ir.rn
    ) AS products_summary
  FROM item_rows ir
  ...
  SELECT
    ...
    string_agg(
      concat(
        ...
        CASE WHEN NULLIF((car.attach_item->>'recommended_timing'), '') IS NOT NULL THEN ' | timing=' || (car.attach_item->>'recommended_timing') ELSE '' END,
        CASE WHEN NULLIF((car.attach_item->>'evidence_strength'), '') IS NOT NULL THEN ' | evidence=' || (car.attach_item->>'evidence_strength') ELSE '' END,
        CASE WHEN NULLIF((car.attach_item->>'attach_trigger'), '') IS NOT NULL THEN E'\\n   trigger: ' || (car.attach_item->>'attach_trigger') ELSE '' END,
        CASE WHEN NULLIF((car.attach_item->>'why_not_in_current_bundle'), '') IS NOT NULL THEN E'\\n   why_not_in_current_bundle: ' || (car.attach_item->>'why_not_in_current_bundle') ELSE '' END,
        CASE WHEN NULLIF((car.attach_item->>'guardrail_note'), '') IS NOT NULL THEN E'\\n   guardrail: ' || (car.attach_item->>'guardrail_note') ELSE '' END
      ),
      E'\\n' ORDER BY car.rn
...
FROM base b
LEFT JOIN products p ON p.opportunity_candidate_id = b.opportunity_candidate_id
LEFT JOIN conditional_attach ca ON ca.opportunity_candidate_id = b.opportunity_candidate_id
ORDER BY b.account, b.version, b.portfolio_priority_score DESC NULLS LAST, b.entity, b.rank_position, b.opportunity_candidate_id
			`);

			const COLUMNS: Array<{ key: string; header: string; width: number }> = [
				{ key: "opportunity_candidate_id", header: "ID", width: 10 },
				{ key: "is_selected", header: "Selected", width: 10 },
				{ key: "account", header: "Account", width: 30 },
				{ key: "entity", header: "Entity", width: 30 },
				{ key: "opportunity_title", header: "Opportunity Title", width: 40 },
				{ key: "rank_position", header: "Rank", width: 8 },
				{
					key: "portfolio_priority_score",
					header: "Priority Score",
					width: 15,
				},
				{
					key: "portfolio_priority_reason",
					header: "Priority Reason",
					width: 50,
				},
				{ key: "horizon", header: "Horizon", width: 15 },
				{ key: "horizon_name", header: "Horizon Name", width: 20 },
				{ key: "horizon_confidence", header: "Horizon Confidence", width: 20 },
				{ key: "horizon_reasoning", header: "Horizon Reasoning", width: 50 },
				{
					key: "indicative_deal_size_range",
					header: "Deal Size Range",
					width: 20,
				},
				{ key: "delivery_type", header: "Delivery Type", width: 18 },
				{ key: "track_1_entity", header: "Track 1 Entity", width: 25 },
				{ key: "track_2_entities", header: "Track 2 Entities", width: 30 },
				{ key: "why_now", header: "Why Now", width: 50 },
				{ key: "notes", header: "Notes", width: 50 },
				{
					key: "primary_business_problem",
					header: "Primary Business Problem",
					width: 40,
				},
				{
					key: "primary_buyer_persona",
					header: "Primary Buyer Persona",
					width: 30,
				},
				{
					key: "primary_value_proposition",
					header: "Primary Value Proposition",
					width: 40,
				},
				{ key: "solution_center", header: "Solution Center", width: 25 },
				{ key: "products_summary", header: "Products", width: 60 },
				{
					key: "conditional_attach_candidate_count",
					header: "Conditional Attach Count",
					width: 10,
				},
				{
					key: "conditional_attach_candidates_summary",
					header: "Conditional Attach Candidates",
					width: 80,
				},
			];
			...
			const normalise = (v: unknown): string | number | boolean | null => {
			...
			for (const row of rows) {
				...
				for (const col of COLUMNS) {
					values[col.key] = normalise(row[col.key]);
				}
			...
			return new NextResponse(buffer, {
				headers: {
					"Content-Type":
						"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
					"Content-Disposition": `attachment; filename="opportunities_${reportId}.xlsx"`,
				},
			});
		} catch (error) {
