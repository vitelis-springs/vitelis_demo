...
export class ReportStepsController {
	...
	static async listGenerationSteps(
		request: NextRequest,
	...
	static async updateGenerationStepSettings(
		...
		try {
			...
			if (
				body.dependency !== undefined &&
				body.dependency !== null &&
				!["rdp", "kpi", "category", "url"].includes(body.dependency)
			) {
				return NextResponse.json(
					{
						success: false,
						error: "dependency must be rdp, kpi, category, url or null",
					},
					{ status: 400 },
				);
			}
			...
			if (
				...
				!["biz_miner", "sales_miner"].includes(body.reportType)
			) {
				return NextResponse.json(
					{
						success: false,
						error: "reportType must be biz_miner, sales_miner or null",
					},
					{ status: 400 },
				);
			}
			...
			const result = await ReportStepsService.updateGenerationStep(stepId, {
