/** biome-ignore-all lint/complexity/noStaticOnlyClass: <фів> */
interface N8NConfig {
	url: string;
	apiKey: string;
}
...
export class N8NService {
	...
	private static getConfigByType(type?: string | null): N8NConfig {
		if (type === "vitelis_sales") {
			return N8NService.getVitelisSalesConfig();
		}
		if (type === "salesminer") {
			return N8NService.getSalesMinerConfig();
		}
		if (type === "bizminer") {
			return N8NService.getBizMinerConfig();
		}
		// Default fallback
		const url = process.env.N8N_API_URL || "https://vitelis.app.n8n.cloud/";
		const apiKey = process.env.N8N_API_KEY;
	...
	static async stopExecution(
		...
		const config = N8NService.getConfigByType(type);
		const url = `${config.url}api/v1/executions/${executionId}/stop`;
		...
		if (!response.ok) {
			throw new Error(
				`N8N stop execution failed with status ${response.status}`,
			);
		}
	}

	static async fetchWithTimeoutPublic(
		url: string,
		options: RequestInit,
		timeoutMs: number = 30000,
	): Promise<Response> {
		return N8NService.fetchWithTimeout(url, options, timeoutMs);
	}

	static getPublicBizMinerConfig(): N8NConfig {
		const url = process.env.N8N_BIZMINER_URL;
		const apiKey = process.env.N8N_BIZMINER_API_KEY || process.env.N8N_API_KEY;
		if (!url || !apiKey)
			throw new Error("N8N BizMiner configuration not found");
	...
	static async startBizMinerWorkflow(
		data: BizMinerWorkflowData,
	): Promise<unknown> {
		const config = N8NService.getBizMinerConfig();
		...
		const response = await N8NService.fetchWithTimeout(triggerUrl, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				"X-N8N-API-KEY": config.apiKey,
	...
	static async startVitelisSalesWorkflow(
		...
		const config = N8NService.getVitelisSalesConfig();
		const triggerUrl = `${config.url}${VITELIS_SALES_PATH_V1}`;
		console.log(
			"🌐 N8NService: Making VitelisSales API request to:",
			triggerUrl,
		);
	...
	static async exportGroupedReport(reportId: number): Promise<Response> {
		const config = N8NService.getConfigByType();
	...
	static async tryQuery(
		...
		const response = await N8NService.fetchWithTimeout(url, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				"X-N8N-API-KEY": config.apiKey,
