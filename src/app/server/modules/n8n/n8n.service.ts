...
export class N8NService {
  ...
  private static getSalesMinerConfig(): N8NConfig {
    ...
    const apiKey = process.env.N8N_SALESMINER_API_KEY || process.env.N8N_API_KEY;
  ...
  static async startSalesMinerWorkflow(data: SalesMinerWorkflowData): Promise<any> {
  ...
  static async getExecutionDetails(executionId: string, type?: string | null): Promise<any> {
    const config = this.getConfigByType(type);
    const url = `${config.url}api/v1/executions/${executionId}?includeData=true`;

    console.log('🔄 N8NService: Fetching execution details for ID:', executionId, 'Type:', type);
    ...
    if (!response.ok) {
      console.error('❌ N8NService: N8N API error:', response.status, response.statusText);
      throw new Error(`N8N API error: ${response.status}`);
    }

    const execution = await response.json();
    console.log(`✅ N8NService: Execution ${executionId} status:`, execution.status);
