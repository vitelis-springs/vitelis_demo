...
export function detectKpiFieldMapping(headers: string[]): Map<KpiFieldKey, number> {
...
export function detectRdpFieldMapping(headers: string[]): Map<RdpFieldKey, number> {
...
function classifyDateFormatCode(code: string): DateFmtInfo | null {
  const clean = code.replace(/"[^"]*"/g, "").replace(/\[[^\]]*\]/g, "").toLowerCase();
...
function resolveAllWorksheetPaths(
  ...
  return Array.from(workbookDoc.getElementsByTagName("sheet"))
    ...
    .filter((item): item is { sheetName: string; worksheetPath: string } => item !== null);
}
...
function parseWorksheet(
  ...
  const rowMaps = rowNodes.map((rowNode) => {
    ...
    cells.forEach((cell) => {
      ...
      if (!ref) return;
      cellMap.set(columnRefToIndex(ref), readCellValue(cell, sharedStrings, dateStyleMap));
    });
    return {
...
export async function parseKpiModelWorkbook(file: File): Promise<KpiModelWorkbook> {
  ...
  for (const { sheetName, worksheetPath } of sheetPaths) {
    ...
    const sheet = parseWorksheet(sheetName, worksheetXml, sharedStrings, dateStyleMap);
    if (sheet) allSheets.push(sheet);
  }

  const norm = (s: string) => s.trim().toLowerCase();
  const kpiSheet =
    allSheets.find((s) => norm(s.sheetName).includes(KPI_SHEET_NAME_PATTERN)) ?? null;
  const rdpSheet =
    allSheets.find((s) => norm(s.sheetName).includes(RDP_SHEET_NAME_PATTERN)) ?? null;
