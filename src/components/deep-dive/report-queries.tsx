...
export default function ReportQueries({
		...
		if (highlightQueryId && queries.length > 0 && !scrolledRef.current) {
			...
			setTimeout(() => {
				const el = document.querySelector(
					`[data-query-id="${highlightQueryId}"]`,
				);
	...
	const summary = useMemo(() => {
		...
		const avgCompletion =
			totalQueries > 0
				? Math.round(
						queries.reduce((sum, q) => sum + q.completionPercent, 0) /
							totalQueries,
					)
				: 0;
	...
	const handleSave = useCallback(async () => {
		...
		if (!editState.goal.trim()) {
			message.warning("Goal cannot be empty");
			return;
		}
				...
				<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
					<div>
						...
						{isEditing ? (
							<Input.TextArea
								...
								onChange={(e) =>
									setEditState((prev) => ({ ...prev, goal: e.target.value }))
								}
								autoSize={{ minRows: 2, maxRows: 6 }}
							/>
						) : (
							<Text style={{ color: "#d9d9d9", whiteSpace: "pre-wrap" }}>
								{query.goal || "—"}
							</Text>
						)}
					</div>
					<div>
						...
						>
							Search Queries (
							{isEditing
								? editState.searchQueries.length
								: query.searchQueries.length}
							)
						</Text>
						{isEditing ? (
					...
					<div>
						...
						<Text style={{ color: "#d9d9d9" }}>
							{query.completedCompanies} / {query.totalCompanies} companies (
