...
function SignalSummaryCards({ rows }: { rows: SalesMinerSignalSummaryRow[] }) {
	if (!rows.length)
		return <Text style={{ color: "#595959" }}>No signal data</Text>;
			...
			{rows.map((r) => (
				<Col key={r.themeCode} xs={24} sm={12} md={8}>
					<Card
						...
						style={{
							...DARK_CARD_STYLE,
							borderLeft: `3px solid ${THEME_COLORS[r.themeCode] ?? "#58bfce"}`,
						}}
					>
						<Text
							style={{
						...
						<Title
							...
							style={{
								margin: 0,
								color: THEME_COLORS[r.themeCode] ?? "#d9d9d9",
							}}
						>
						...
						<Text style={{ color: "#595959", fontSize: 12 }}>
							...
							{r.avgStrength !== null
								? ` · avg strength ${(r.avgStrength * 100).toFixed(0)}%`
								: ""}
						</Text>
					</Card>
				</Col>
	...
	const colTitle = (label: string, hint: string) => (
		<Space size={4}>
			...
			<Tooltip title={hint} placement="top">
				<QuestionCircleOutlined
					style={{ color: "#595959", fontSize: 12, cursor: "help" }}
				/>
			</Tooltip>
		</Space>
	);
	...
	const columns = useMemo(() => {
			...
			},
			{
				...
				render: (v: number | null) =>
					v !== null ? (
						<Text
							style={{
								color: v >= 65 ? "#52c41a" : v >= 55 ? "#faad14" : "#8c8c8c",
							}}
						>
							{v.toFixed(1)}
						</Text>
		...
		if (showSignals) {
			cols.push({
				...
				render: (v: number) => (
					<Badge count={v} color="#13c2c2" overflowCount={999} />
				),
			});
		}
		...
		<>
			...
			<Row gutter={[16, 16]} style={{ marginBottom: 24 }} align="stretch">
				<Col xs={24} sm={8} style={{ display: "flex" }}>
					...
					>
						<Text style={{ color: "#8c8c8c", display: "block", fontSize: 12 }}>
							Analyzed
						</Text>
			...
			<Row gutter={[16, 16]} style={{ marginBottom: 24 }} align="stretch">
				...
				{isV2 && (
					<Col xs={24} sm={8} style={{ display: "flex" }}>
						...
						>
							<Text
								style={{ color: "#8c8c8c", display: "block", fontSize: 12 }}
							>
								Total Signals
							</Text>
							<Title level={3} style={{ margin: 0, color: "#13c2c2" }}>
...
export default function SalesMinerDetail({ reportId }: { reportId: number }) {
	...
	const { data: smData, isLoading: smLoading } =
	...
	const accountVersion =
		typeLevel === "account"
			? ((smData?.data as { accountVersion?: string })?.accountVersion ?? "1")
			: null;
					...
					<div style={{ marginBottom: 24 }}>
						...
						<div style={{ marginTop: 8 }}>
							...
							>
								...
								{showLevelTag && (
									<Tag
										color={typeLevel === "account" ? "purple" : "cyan"}
							...
							{report?.description && (
								<Text
									style={{ color: "#8c8c8c", marginTop: 4, display: "block" }}
								>
									{report.description}
								</Text>
							)}
