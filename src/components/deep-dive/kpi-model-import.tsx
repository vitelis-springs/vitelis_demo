...
function parseDependencies(raw: string | null, prefix: number): string[] {
  ...
  if (Number.isFinite(asNumber) && asNumber > 0) {
    if (asNumber >= EXCEL_DATE_SERIAL_MIN) {
      ...
      return excelSerialToParts(asNumber).map((n) => `raw_data_point_${n + prefix}`);
    }
...
function buildKpiDataPoints(
  ...
  const getIndex = (key: KpiFieldKey) =>
    resolveColumnIndex(sheet.headers, autoMap as Map<string, number>, userMapping, key);

  const numberIdx = getIndex("number");
  const requiredIndices = getRequiredIndices(sheet, KPI_FIELD_LABELS, autoMap, userMapping);

  return sheet.rows
    .map((row) => {
      const rawNumber = numberIdx !== undefined ? (row.values[numberIdx] ?? null) : null;
...
function buildRdpDataPoints(
  ...
  const numberIdx = getIndex("number");
  const requiredIndices = getRequiredIndices(sheet, RDP_FIELD_LABELS, autoMap, userMapping);
  ...
  return sheet.rows
    .map((row) => {
      ...
      return { id: dataPointId, type: "raw_data_point" as const, name, settings };
    })
    .filter((item): item is NonNullable<typeof item> => item !== null);
}
...
function FieldMappingHeader<TKey extends string>({
  ...
  const isMapped =
    userOverride !== undefined ? headers.includes(userOverride) : autoMappingIndex !== undefined;
    ...
    <div style={{ whiteSpace: "normal", wordBreak: "break-word", minWidth: 0 }}>
      <Space size={4} align="start" style={{ flexWrap: "nowrap" }}>
        {isMapped ? (
          ...
          <WarningOutlined style={{ color: "#faad14", fontSize: 11, marginTop: 2 }} />
        )}
        <Tooltip title={isMapped ? undefined : `"${label}" not found. Click ↺ to map.`}>
          <Text style={{ color: isMapped ? "#d9d9d9" : "#faad14", fontSize: 12 }}>
            {label}
          </Text>
        </Tooltip>
        <RemapPopover
...
function buildPreviewColumns<TKey extends string>(
  ...
  const requiredKeys = (Object.keys(fieldLabels) as TKey[]).filter((k) => k !== ("number" as TKey));
  ...
  const idColumn = {
    ...
    render: (_: unknown, row: { values: (string | null)[] }) => {
      const rawNum = numberIdx !== undefined ? (row.values[numberIdx] ?? null) : null;
      const num = rawNum !== null ? Number(rawNum) : null;
      if (!Number.isFinite(num) || num === null) {
        return <Text style={{ color: "#ff7875", fontSize: 11 }}>invalid #</Text>;
      }
  ...
  const requiredColumns = requiredKeys.map((key) => {
    ...
    return {
      ...
      ellipsis: true,
      render: (_: unknown, row: { values: (string | null)[] }) => {
        if (idx === undefined) return <Text style={{ color: "#595959", fontSize: 12 }}>—</Text>;
        const val = row.values[idx];
        if (!val) return <Text style={{ color: "#595959", fontSize: 12 }}>—</Text>;
  ...
  const optionalColumns = sheet.headers
    ...
    .filter(({ header, colIdx }) => header.trim() !== "" && !requiredIndices.has(colIdx))
    .map(({ header, colIdx }) => {
      ...
      return {
        title: (
          <div style={{ whiteSpace: "normal", wordBreak: "break-word", minWidth: 0 }}>
            <Space size={4} align="start" style={{ flexWrap: "nowrap" }}>
              ...
              />
              <Text style={{ color: included ? "#8c8c8c" : "#434343", fontSize: 12 }}>
                {header}
              </Text>
            </Space>
          </div>
        ...
        render: (_: unknown, row: { values: (string | null)[] }) => {
          ...
          return (
            <Tooltip title={val} placement="topLeft">
              <Text style={{ color: included ? "#8c8c8c" : "#434343", fontSize: 12 }} ellipsis>
                {val}
              </Text>
            </Tooltip>
          );
    ...
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      <Space wrap>
        ...
        {missingRequired.length > 0 && (
          <Tag color="warning" icon={<WarningOutlined />}>
            {missingRequired.length} required field{missingRequired.length > 1 ? "s" : ""} not
            mapped
          </Tag>
        )}
        {excludedOptionals.size > 0 && (
          <Tag color="default">{excludedOptionals.size} optional field{excludedOptionals.size > 1 ? "s" : ""} excluded</Tag>
        )}
      </Space>
      ...
      {missingRequired.length > 0 && (
        <Alert
          ...
          description={
            <Text>
              Click the <RetweetOutlined style={{ color: "#faad14" }} /> button next to
              each highlighted field header to map it to a column from the imported file.
            </Text>
          }
        />
      )}
...
const KpiModelImport = forwardRef<KpiModelImportHandle, KpiModelImportProps>(function KpiModelImport({
  reportId,
  prefix,
  existingItems,
  onClose,
  ...
  const [kpiExcludedOptionals, setKpiExcludedOptionals] = useState<Set<string>>(new Set());
  const [rdpExcludedOptionals, setRdpExcludedOptionals] = useState<Set<string>>(new Set());
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if ((incomingHasKpi || incomingHasRdp) && reportHasKpi && reportHasRdp) {
        startTransition(() => {
          setWorkbook(null);
          setFileName(null);
          setKpiUserMapping({});
      ...
      startTransition(() => {
        ...
        setKpiExcludedOptionals(new Set());
        setRdpExcludedOptionals(new Set());
        setActiveTab(nextWorkbook.kpiSheet ? "kpi" : nextWorkbook.rdpSheet ? "rdp" : "kpi");
      });
      ...
      if (!sheets.length) {
        ...
        void message.success(`Parsed sheets: ${sheets.join(", ")} from ${file.name}`);
        onFileDialogSettled?.();
      }
    } catch (err) {
      void message.error(err instanceof Error ? err.message : "Failed to parse XLSX file");
      onFileDialogSettled?.();
    } finally {
      setIsParsing(false);
    }
  ...
  const buildImportDataPoints = (type: "kpi" | "rdp" | "all"): ImportKpiModelPayload["dataPoints"] => {
    if (!workbook) return [];
  ...
  const handleApply = async (type: "kpi" | "rdp" | "all") => {
    ...
    try {
      ...
      void message.success(`Imported ${dataPoints.length} ${importedLabel} data points`);
      onImported?.();
    } catch (err) {
      const msg =
          ...
          ? [
              {
                ...
                label: (
                  <Space size={6}>
                    ...
                    <Text style={{ color: "#8c8c8c" }}>{workbook.kpiSheet.rows.length}</Text>
                  </Space>
                ),
                children: (
                  <Space direction="vertical" size="middle" style={{ width: "100%" }}>
                    ...
                    <SheetPreview
                      ...
                      cellRenderers={{
                        dependencies: (val) => {
                          ...
                          if (!ids.length) return <Text style={{ color: "#595959", fontSize: 12 }}>—</Text>;
                          return (
                            <Space size={2} wrap>
                              {ids.map((id) => (
                                <Tag key={id} style={{ fontSize: 11, margin: 0 }} color="green">
                                  {id}
                                </Tag>
                              ))}
                            </Space>
                      ...
                      onToggleOptional={(header, included) =>
                        toggleOptional(setKpiExcludedOptionals, header, included)
                      }
          ...
          ? [
              {
                ...
                label: (
                  <Space size={6}>
                    ...
                    <Text style={{ color: "#8c8c8c" }}>{workbook.rdpSheet.rows.length}</Text>
                  </Space>
                ),
                children: (
                  <Space direction="vertical" size="middle" style={{ width: "100%" }}>
                    ...
                    <SheetPreview
                      ...
                      onToggleOptional={(header, included) =>
                        toggleOptional(setRdpExcludedOptionals, header, included)
                      }
  ...
  return (
    <>
      ...
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        {(reportHasKpi || reportHasRdp) && !(reportHasKpi && reportHasRdp) && (
          <Alert
            type="warning"
            showIcon
            ...
            description={
              <Space direction="vertical" size={2}>
                {reportHasKpi && (
                  <Text>KPI sheets will not be opened from XLSX because this report already contains KPI.</Text>
                )}
                {reportHasRdp && (
                  <Text>RDP sheets will not be opened from XLSX because this report already contains RDP.</Text>
                )}
                {reportHasKpi && reportHasRdp && (
                  <Text>Import is unavailable for this report. Update KPI Model manually.</Text>
                )}
              </Space>
            }
          />
        ...
        {workbook && !workbook.kpiSheet && !workbook.rdpSheet && (
          <Alert
            ...
            description={
              <Space direction="vertical" size={2}>
                <Text>
                  Expected sheets containing <Text code>{KPI_SHEET_NAME_PATTERN}</Text> or{" "}
