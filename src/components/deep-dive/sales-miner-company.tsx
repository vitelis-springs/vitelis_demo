  ...
  return (
    <div style={{ marginBottom: 4 }}>
      ...
      <Progress
        ...
        format={(p) => <Text style={{ fontSize: 11, color: "#d9d9d9" }}>{p}%</Text>}
      />
    </div>
  );
}
      ...
      ? [
          {
            ...
            key: "entityName",
            width: 200,
            render: (v: string | undefined) => <Text style={{ fontSize: 12 }}>{v ?? "—"}</Text>,
          },
        ]
      : []),
    {
          ...
          <div style={{ padding: "8px 16px" }}>
            {record.whyNow && (
              <div style={{ marginBottom: 8 }}>
                <Text strong style={{ color: "#faad14" }}>Why now: </Text>
                <Text style={{ color: "#d9d9d9" }}>{record.whyNow}</Text>
              </div>
            )}
            {record.businessProblem && (
              <div style={{ marginBottom: 8 }}>
                <Text strong style={{ color: "#8c8c8c" }}>Business problem: </Text>
                <Text style={{ color: "#d9d9d9" }}>{record.businessProblem}</Text>
              </div>
            )}
            {record.valueProposition && (
              <div>
                <Text strong style={{ color: "#8c8c8c" }}>Value proposition: </Text>
                <Text style={{ color: "#d9d9d9" }}>{record.valueProposition}</Text>
              </div>
            )}
          </div>
        ),
...
function SignalsGrid({
      ...
      <Row gutter={[16, 16]}>
        {items.map((s) => (
          <Col xs={24} lg={12} key={s.id}>
            <Card size="small" style={DARK_CARD} title={
              <Text strong style={{ color: "#d9d9d9" }}>{s.signalName ?? "Signal"}</Text>
            }>
              {s.signalDescription && (
              ...
              {s.summaryText && (
                <Paragraph style={{ color: "#d9d9d9", fontSize: 12, marginTop: 8, marginBottom: 0 }}>
                  {s.summaryText}
                </Paragraph>
              )}
            </Card>
  ...
  return (
    <Tabs
      items={[
        { key: "micro", label: `Micro signals (${groups.micro.length})`, children: renderCards(groups.micro) },
        { key: "macro", label: `Macro signals (${groups.macro.length})`, children: renderCards(groups.macro) },
        { key: "product", label: `Product signals (${groups.product.length})`, children: renderCards(groups.product) },
      ]}
    />
  );
}
...
function AccountLevelView({
  ...
  const snapshot = useMemo(() => {
    ...
    const stage1 = toRecord(toRecord(items[1]).account_snapshot_stage_1).account as Record<string, unknown> | undefined;
    const stage2 = toRecord(toRecord(items[0]).account_snapshot_stage_2_group_strategy).account as Record<string, unknown> | undefined;
  ...
  const leadPlays = toArray(assessment?.lead_plays) as Array<Record<string, unknown>>;
  const stakeholders = toArray(assessment?.stakeholders) as Array<Record<string, unknown>>;
  const timingSignals = toArray(assessment?.timing_signals) as Array<Record<string, unknown>>;
  const itThemes = toArray(snapshot?.stage1?.it_themes) as string[];
  const signals = toArray(snapshot?.stage1?.signals) as Array<Record<string, unknown>>;
  const strategicInitiatives = toArray(snapshot?.stage1?.strategic_initiatives_and_themes) as Array<Record<string, unknown>>;
  const groupItStrategy = toArray(snapshot?.stage2?.group_it_strategy) as string[];
  const latestNews = toArray(snapshot?.stage2?.latest_news_on_goals) as string[];
  ...
  const stakeholderRows: SalesMinerStakeholder[] = stakeholders.map((s, i) => ({
    ...
    rationale: toString(s.notes_how_to_approach) || toString(s.likely_scope) || null,
    opportunityId: null,
  }));
    ...
    <DeepDivePageLayout>
      ...
      {itThemes.length > 0 && (
        <Space size={4} wrap style={{ marginBottom: 24 }}>
          {itThemes.map((t) => <Tag key={t} color="geekblue">{t}</Tag>)}
        </Space>
      )}
      ...
      <Tabs
        items={[
              ...
              <div>
                ...
                {signals.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Signals</Title>
                    <Row gutter={[12, 12]}>
                      {signals.map((s, i) => (
                        <Col xs={24} lg={12} key={i}>
                          <Card size="small" style={DARK_CARD}>
                            <Text strong style={{ color: "#d9d9d9" }}>{toString(s.signal)}</Text>
                            <div style={{ marginTop: 4 }}>
                              <Text style={{ color: "#faad14", fontSize: 12 }}>Why it matters: </Text>
                              <Text style={{ color: "#8c8c8c", fontSize: 12 }}>{toString(s.why_it_matters)}</Text>
                            </div>
                            {!!s.likely_plays && (
                              <div style={{ marginTop: 4 }}>
                                <Text style={{ color: "#52c41a", fontSize: 12 }}>Likely plays: </Text>
                                <Text style={{ color: "#8c8c8c", fontSize: 12 }}>{toString(s.likely_plays)}</Text>
                              </div>
                            )}
                          </Card>
                        </Col>
                ...
                {strategicInitiatives.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Strategic Initiatives</Title>
                    <Collapse
                      items={strategicInitiatives.map((s, i) => ({
                        key: i,
                        label: <Text style={{ color: "#d9d9d9" }}>{toString(s.label)}</Text>,
                        children: (
                          <div>
                            <Paragraph style={{ color: "#8c8c8c" }}>{toString(s.rationale_text)}</Paragraph>
                            {toArray(s.context_anchors).length > 0 && (
                              <ul style={{ color: "#8c8c8c", paddingLeft: 20 }}>
                                {(toArray(s.context_anchors) as string[]).map((a, j) => (
                                  <li key={j} style={{ marginBottom: 4 }}>{a}</li>
                                ))}
                              </ul>
                            )}
                          </div>
                ...
                {groupItStrategy.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Group IT Strategy</Title>
                    <ul style={{ color: "#8c8c8c", paddingLeft: 20 }}>
                      {groupItStrategy.map((item, i) => <li key={i} style={{ marginBottom: 6 }}>{item}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {latestNews.length > 0 && (
                  <div>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Latest News</Title>
                    <ul style={{ color: "#8c8c8c", paddingLeft: 20 }}>
                      {latestNews.map((item, i) => <li key={i} style={{ marginBottom: 6 }}>{item}</li>)}
                    </ul>
                  </div>
                )}
              </div>
          ...
          },
          {
            key: "opportunities",
            label: (
              <Badge count={topOpportunities.length} overflowCount={99} color="#1677ff" offset={[6, -2]}>
          ...
          },
          {
            ...
            children: (
              <div>
                ...
                {pains.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Pains</Title>
                    <Row gutter={[12, 12]}>
                      {pains.map((p, i) => (
                        <Col xs={24} lg={12} key={i}>
                          <Card
                            size="small"
                            style={{ ...DARK_CARD, borderLeft: "3px solid #1677ff" }}
                            title={<Text strong style={{ color: "#d9d9d9" }}>{toString(p.label)}</Text>}
                          >
                            <Paragraph style={{ color: "#8c8c8c", fontStyle: "italic", marginBottom: 8 }}>
                              &quot;{toString(p.rationale_text)}&quot;
                            </Paragraph>
                            {toArray(p.context_anchors).length > 0 && (
                              <ul style={{ color: "#595959", fontSize: 12, paddingLeft: 16, margin: 0 }}>
                                {(toArray(p.context_anchors) as string[]).slice(0, 3).map((a, j) => (
                                  <li key={j}>{a}</li>
                                ))}
                              </ul>
                            )}
                ...
                {leadPlays.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Lead Plays</Title>
                    <Table
                          ...
                          <div style={{ padding: "8px 16px" }}>
                            {!!r.why_now_trigger && (
                              <div style={{ marginBottom: 6 }}>
                                <Text strong style={{ color: "#faad14" }}>Why now: </Text>
                                <Text style={{ color: "#8c8c8c", fontSize: 12 }}>{toString(r.why_now_trigger)}</Text>
                              </div>
                            )}
                            {!!r.first_next_step && (
                              <div>
                                <Text strong style={{ color: "#52c41a" }}>First step: </Text>
                                <Text style={{ color: "#8c8c8c", fontSize: 12 }}>{toString(r.first_next_step)}</Text>
                              </div>
                            )}
                          </div>
                        ),
                ...
                {timingSignals.length > 0 && (
                  <div>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Timing</Title>
                    <Space direction="vertical" style={{ width: "100%" }}>
                      {timingSignals.map((t, i) => (
                        <Card key={i} size="small" style={DARK_CARD}>
                          <Space>
                            <Tag color={HORIZON_COLORS[toString(t.timing_label)] ?? "default"}>
                              {toString(t.time_horizon)}
                            </Tag>
                            <Text style={{ color: "#d9d9d9" }}>{toString(t.theme)}</Text>
                          </Space>
                          {!!t.notes_evidence && (
                            <Paragraph style={{ color: "#8c8c8c", fontSize: 12, marginTop: 6, marginBottom: 0 }}>
          ...
          },
          {
            key: "stakeholders",
            label: (
              <Badge count={stakeholderRows.length} overflowCount={99} color="purple" offset={[6, -2]}>
                <span>Stakeholders</span>
              </Badge>
            ),
            children: <StakeholdersTable stakeholders={stakeholderRows} loading={isLoading} />,
          },
          {
            key: "seller-brief",
            label: "Seller Brief",
              ...
              <div>
                {!!sellerBrief.role_ops && (
                  <div style={{ marginBottom: 16 }}>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Entry Points</Title>
                    <Paragraph style={{ color: "#8c8c8c" }}>{toString(sellerBrief.role_ops)}</Paragraph>
                  </div>
                )}
                {toArray(sellerBrief.lead_plays).length > 0 && (
                  <div>
                    <Title level={5} style={{ color: "#d9d9d9" }}>Lead Plays</Title>
                    <ul style={{ color: "#8c8c8c", paddingLeft: 20 }}>
                      {(toArray(sellerBrief.lead_plays) as string[]).map((p, i) => (
        ...
        items={[
          {
            key: "signals",
            label: (
              <Badge count={signals.length} overflowCount={99} color="cyan" offset={[6, -2]}>
                <span>Signals</span>
              </Badge>
            ),
          ...
          },
          {
            key: "opportunities",
            label: (
              <Badge count={opportunities.length} overflowCount={99} color="#1677ff" offset={[6, -2]}>
                <span>Opportunities</span>
              </Badge>
            ),
            children: (
              <OpportunitiesTable opportunities={opportunities} loading={isLoading} />
            ),
          },
          {
            key: "stakeholders",
            label: (
              <Badge count={stakeholders.length} overflowCount={99} color="purple" offset={[6, -2]}>
                <span>Stakeholders</span>
              </Badge>
            ),
            children: <StakeholdersTable stakeholders={stakeholders} loading={isLoading} />,
          },
        ]}
      />
    </DeepDivePageLayout>
