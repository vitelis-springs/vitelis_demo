...
import { DeleteOutlined, EditOutlined, ImportOutlined, ReloadOutlined } from "@ant-design/icons";
import {
  App,
  Button,
  Card,
...
interface ReportModelItemModalProps {
  ...
  onCancel: () => void;
  onSubmit: (payload: { name: string; settings: Record<string, unknown> }) => Promise<void>;
}
...
function getEmptyValueFromTemplate(value: unknown, fallbackNumber?: number): unknown {
...
function ReportModelItemModal({
  ...
  const [settingsObj, setSettingsObj] = useState<Record<string, unknown>>(initialSettings);
                ...
                if (typeof value === "boolean") {
                  return (
                    <div key={inputKey}>
                      ...
                      <Switch
                        checked={value}
                        onChange={(checked) => handleSettingsFieldChange(key, checked)}
                      />
                    </div>
                  );
                }
                ...
                return (
                  <div key={inputKey}>
                    ...
                    <TextArea
                      ...
                      onChange={(event) => {
                        try {
                          handleSettingsFieldChange(key, JSON.parse(event.target.value));
                        } catch {
                          handleSettingsFieldChange(key, event.target.value);
                        }
                      }}
...
export default function ReportModelPage({
    ...
    const warningContent =
      dependentKpis.length > 0 ? (
        <Space direction="vertical" size={4}>
          <Text>
            This will remove the item from the report model and delete its results for all
            companies in this report.
          </Text>
          <Text style={{ color: "#faad14" }}>
            ⚠ The following KPI drivers reference this RDP in their Dependencies:
          </Text>
          <ul style={{ margin: "4px 0 0 16px", padding: 0 }}>
            {dependentKpis.map((kpi) => (
              <li key={kpi.dataPointId}>
                ...
                {kpi.name ? (
                  <Text style={{ color: "#8c8c8c", marginLeft: 8, fontSize: 12 }}>
                    {kpi.name}
                  </Text>
                ) : null}
              </li>
  ...
  const handleOpenCreate = (type: "kpi_driver" | "raw_data_point") => {
    const templateItem = type === "kpi_driver" ? kpiTemplateItem : rdpTemplateItem;
    ...
    const nextDataPointId =
      type === "kpi_driver" ? `kpi_driver_${nextIndex}` : `raw_data_point_${nextIndex}`;
  ...
  return (
    <DeepDivePageLayout maxWidth="none">
      ...
      <ReportModelItemModal
        ...
        title={editingItem ? `Edit ${editingItem.dataPointId}` : "Edit model item"}
        okText="Save"
        confirmLoading={updateModelItem.isPending}
        dataPointId={editingItem?.type === "kpi_driver" ? editingItem.dataPointId : undefined}
        initialName={editingItem?.name ?? ""}
        initialSettings={editingItem?.settings ?? {}}
        requiredFields={
          editingItem?.type === "kpi_driver" ? [...REQUIRED_KPI_CREATE_FIELDS] : []
        }
      ...
      <ReportModelItemModal
        ...
        requiredFields={
          creatingItem?.type === "kpi_driver" ? [...REQUIRED_KPI_CREATE_FIELDS] : []
        }
