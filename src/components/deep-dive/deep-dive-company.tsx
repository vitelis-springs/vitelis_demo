...
const toSourcesText = (value: unknown): string =>
...
export default function DeepDiveCompany({
	...
	const handleSaveDataPoint = useCallback(
	...
	const { categoryRows, driverRows, rawRows } = useMemo(() => {
		...
		for (const result of kpiResults) {
			...
			if (isCategory) {
				...
				const kpiName =
					getString(dr, "Definition (KPI)") || getString(dr, "KPI") || "—";
				const driverName =
	...
	const matchesSearch = useCallback(
		(search: string, ...fields: (string | null | undefined)[]) => {
		...
		(): TableColumnsType<CategoryRow> => [
			...
			},
			{
				...
				sorter: (a: CategoryRow, b: CategoryRow) =>
					a.category.localeCompare(b.category),
				render: (v: string) => <MarkdownCell extended>{v}</MarkdownCell>,
			},
			{
				title: "Score",
			...
			},
			{
				...
				dataIndex: "reasoning",
				width: 600,
				render: (v: string | null) => <MarkdownCell extended>{v}</MarkdownCell>,
			},
			{
				title: "Actions",
				key: "actions",
	...
	const driverColsDef = useMemo(
		(): TableColumnsType<DriverRow> => [
			...
			},
			{
				...
				dataIndex: "category",
				width: 140,
				sorter: (a: DriverRow, b: DriverRow) =>
					a.category.localeCompare(b.category),
				render: (v: string) => <MarkdownCell extended>{v}</MarkdownCell>,
			},
			{
	...
	return (
		<DeepDivePageLayout>
			...
			<Space size="middle" wrap style={{ marginBottom: 24 }}>
				...
				{payload?.company.investPortal && (
					<Text style={{ color: "#8c8c8c" }}>
						Invest:{" "}
						<a
							href={payload.company.investPortal}
							target="_blank"
							rel="noopener noreferrer"
							style={{ color: "#58bfce" }}
						>
				...
				{payload?.company.careerPortal && (
					<Text style={{ color: "#8c8c8c" }}>
						Careers:{" "}
						<a
							href={payload.company.careerPortal}
							target="_blank"
							rel="noopener noreferrer"
							style={{ color: "#58bfce" }}
						>
			...
			<Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
				<Col xs={24} md={8}>
					<StatCard
						label="Total sources"
						value={sourcesTotal}
						href={`${basePath}/${reportId}/companies/${companyId}/sources`}
					/>
				</Col>
				<Col xs={24} md={8}>
					<StatCard
						label="Scrape candidates"
						value={scrapCandidatesTotal}
						href={`${basePath}/${reportId}/companies/${companyId}/candidates`}
					/>
				</Col>
				<Col xs={24} md={8}>
					...
					<div style={{ height: 480 }}>
						<ResponsiveContainer width="100%" height="100%">
							<RadarChart data={radarData} cx="50%" cy="50%" outerRadius="75%">
								...
								<PolarAngleAxis
									dataKey="category"
									tick={{ ...CHART_AXIS_TICK_STYLE, fontSize: 13 }}
								/>
								<PolarRadiusAxis
									domain={SCORE_DOMAIN}
					...
					items={[
						{
							...
							children: (
								<div>
									...
									{categorySearch && (
										<Text
											style={{ color: "#8c8c8c", marginLeft: 12, fontSize: 12 }}
										>
