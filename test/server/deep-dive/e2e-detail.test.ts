...
jest.mock("../../../src/lib/prisma", () => {
	...
	return {
		...
		default: {
			...
			report_data_point_results: { findMany: rdp },
...
const {
	_fu: mockFindUnique,
	_fm: mockFindMany,
	_rdp: mockReportDataPointResultsFindMany,
...
function setupDetailMocks(report = SAMPLE_REPORT) {
	...
	mockQueryRaw
		...
		.mockResolvedValueOnce([
			{ company_id: 100, total: 3, valid_count: 2 },
			{ company_id: 200, total: 2, valid_count: 2 },
		])
		.mockResolvedValueOnce([
...
describe("E2E: GET /api/deep-dive/[id]", () => {
	...
	it("returns full report detail with KPI chart data", async () => {
		...
		expect(body.data.report.useCase).toEqual({
			id: 2,
		...
		expect(body.data.summary.usedSources).toBe(3);
		...
		const acme = body.data.companies.find(
			(c: { name: string }) => c.name === "Acme Corp",
		);
