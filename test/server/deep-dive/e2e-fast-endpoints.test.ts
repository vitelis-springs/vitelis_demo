...
jest.mock("../../../src/lib/prisma", () => {
	...
	return {
		...
		default: {
			...
			report_companies: { findMany: rcfm, count: rcc },
			report_data_collection_queries: { count: rdcqc },
			report_data_point_results: { findMany: rdprfm },
...
const {
	_fu: mockFindUnique,
	_rcfm: mockReportCompaniesFindMany,
	_rcc: mockReportCompaniesCount,
	_rdcqc: mockReportQueriesCount,
	_rdprfm: mockReportDataPointResultsFindMany,
...
describe("E2E: GET /api/deep-dive/[id]/companies", () => {
	...
	it("returns companies table independently from summary endpoint", async () => {
		mockFindUnique.mockResolvedValueOnce(SAMPLE_REPORT);
		mockReportCompaniesFindMany.mockResolvedValueOnce([
			{
				company_id: 100,
				companies: {
					id: 100,
		...
		mockReportDataPointResultsFindMany.mockResolvedValueOnce([
			{
				company_id: 100,
				data: {
					sources: ["https://acme.com/source-a", "https://acme.com/source-b/"],
				},
			},
			{
				company_id: 100,
				data: {
					Sources: "1. https://acme.com/source-a\nhttps://acme.com/source-c",
				},
			},
			{
				company_id: 200,
				data: {
					sources: [
						{ url: "https://beta.de/source-1/" },
						{ url: "https://beta.de/source-2" },
					],
				},
			},
		]);
