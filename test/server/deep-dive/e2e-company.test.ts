...
const SAMPLE_MANUAL_MODEL_ITEMS = [
	{
...
function setupCompanyMocks() {
	jest
		...
		.mockResolvedValueOnce(SAMPLE_COMPANY as never);
	jest.spyOn(DeepDiveRepository, "getReportById").mockResolvedValueOnce(null);
	jest
		.spyOn(DeepDiveRepository, "getReportSteps")
		.mockResolvedValueOnce(SAMPLE_STEPS as never);
	jest
		.spyOn(DeepDiveRepository, "getCompanyStepStatuses")
		.mockResolvedValueOnce(SAMPLE_STEP_STATUSES as never);
	jest
		.spyOn(DeepDiveRepository, "getCompanyKpiResults")
		.mockResolvedValueOnce(SAMPLE_KPI_RESULTS as never);
	jest
		.spyOn(DeepDiveRepository, "getCompanyScrapCandidates")
		.mockResolvedValueOnce(SAMPLE_CANDIDATES as never);
	jest
		.spyOn(DeepDiveRepository, "getCompanyScrapCandidatesCount")
	...
	jest
		.spyOn(DeepDiveRepository, "getKpiCategoryScoresByCompany")
		.mockResolvedValueOnce(SAMPLE_KPI_ALL_SCORES);
	jest
		.spyOn(DeepDiveRepository, "getManualReportModelItems")
		.mockResolvedValueOnce(SAMPLE_MANUAL_MODEL_ITEMS as never);
}
...
describe("E2E: GET /api/deep-dive/[id]/companies/[companyId]", () => {
	...
	it("returns full company detail with steps, KPIs, sources, candidates", async () => {
		...
		expect(body.data.manualDataPoints).toHaveLength(1);
		expect(body.data.manualDataPoints[0].dataPointId).toBe("raw_data_point_1");
		expect(body.data.manualDataPoints[0].resultId).toBeNull();
	...
	it("parses query params for source filtering", async () => {
		const sourcesSpy = jest
			.spyOn(DeepDiveRepository, "getCompanySources")
		...
		await callGET(
			"10",
			"173",
			"?sourcesLimit=20&sourcesOffset=5&tier=2&isVectorized=true",
		);
