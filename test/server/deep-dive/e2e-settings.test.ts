...
describe("E2E: GET /api/deep-dive/[id]/settings", () => {
	...
	it("returns 404 when report does not exist", async () => {
		...
		jest
			.spyOn(DeepDiveRepository, "listAllUseCases")
			.mockResolvedValueOnce(USE_CASES);
		jest
			.spyOn(DeepDiveRepository, "listAllCountries")
			.mockResolvedValueOnce(COUNTRIES);
		jest
			.spyOn(DeepDiveRepository, "getReportCountryIds")
	...
	it("returns current settings and available options", async () => {
		...
		expect(body.data.countries.all).toHaveLength(2);
		expect(body.data.countries.selected).toEqual(["US"]);
	});
});
...
describe("E2E: PATCH /api/deep-dive/[id]/settings", () => {
	...
	it("updates existing settings in place", async () => {
		const snapshotAfter = {
			...
			reportSettings: {
				...SNAPSHOT_INITIAL.reportSettings,
				settings: { k: 1 },
			},
			validatorSettings: {
				...SNAPSHOT_INITIAL.validatorSettings,
		...
		expect(updateBasicSpy).toHaveBeenCalledWith(
			44,
			expect.objectContaining({ name: "Updated Name" }),
		);
		expect(updateRsSpy).toHaveBeenCalledWith(
			1,
			expect.objectContaining({ settings: { k: 1 } }),
		);
		expect(updateVsSpy).toHaveBeenCalledWith(
			10,
			expect.objectContaining({ settings: { minScore: 0.8 } }),
		);
	});
